<template>
  <q-page>
    ...
    <q-dialog v-model="saveMadal" transition-show="slide-up" transition-hide="slide-down">
       <add-acteur :infoPere="itemParcelle"/>
    </q-dialog>
...
import AddActeur from "components/Acteur/Parcelliser.vue";
export default defineComponent({
  ...
  setup(props) {
    ...
    onMounted(async () =>{
       ...
       /* Polygone et centriod */
      let infoParcelle = JSON.parse(props.itemParcelle)
      console.log("infoParcelle",infoParcelle.acteurPoly[0]);
      let centroid = JSON.parse(infoParcelle.acteurPoly[0].centroid)
      let geometrie = JSON.parse(infoParcelle.acteurPoly[0].geometrie)

      console.log("centroid ==>", centroid);
      console.log("geometrie ==>", geometrie);

      let coords;
      if (geometrie.features) {
        coords = geometrie.features[0].geometry.coordinates
        console.log("coordinate 1 ==> ", coords);
      }

      if (geometrie.coordinates) {
        coords = geometrie.coordinates
        console.log("coordinate 2 ==> ", coords);
      }
      ...
      var mymap = L.map('mapid', {
          zoomControl: false, 
      }).setView([ coordinate.value.latitude,  coordinate.value.longitude], 14);
      ...
      L.geoJSON(geometrie).addTo(mymap)
      mymap.flyTo(new L.LatLng(coordinate.value.latitude , coordinate.value.longitude), 16);
