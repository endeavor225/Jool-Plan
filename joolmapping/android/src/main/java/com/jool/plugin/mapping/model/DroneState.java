...
public class DroneState {
  ...
  @Nullable OnStateChangeListener onRecordLog;
              ...
              if (getAircraft().getModel() == Model.UNKNOWN_AIRCRAFT) {
                ...
                if (Utils.isNull(controller)){
                  ...
                  if (Utils.isNull(state)){
                    ...
                    Double uLatitude = LocationManager.getuLatitude();
                    Double uLongitude = LocationManager.getuLongitude();
                    float distanceInMeter = 0;
                    if (Utils.isNull(uLatitude) || Utils.isNull(uLongitude)){
                      // Skip
                    }else {
                      if (uLatitude.isNaN() || uLongitude.isNaN()){
                        // Skip
                      }else {
                        double droneLongitude = getLongitude();
                        double droneLatitude = getLatitude();
                        if (uLatitude == Constants.DOUBLE_NULL || uLatitude == Constants.DOUBLE_NULL_2
                        || uLongitude == Constants.DOUBLE_NULL || uLongitude == Constants.DOUBLE_NULL_2
                        ||  droneLongitude == Constants.DOUBLE_NULL || droneLongitude == Constants.DOUBLE_NULL_2
                        || droneLatitude == Constants.DOUBLE_NULL || droneLatitude == Constants.DOUBLE_NULL_2){
                          // Skip
                        }else {
                          ...
                          droneLocation.setLatitude(droneLatitude);
                          droneLocation.setLongitude(droneLongitude);
                          distanceInMeter = uLocation.distanceTo(droneLocation);
