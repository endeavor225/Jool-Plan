...
public class MissionSetting {
    ...
    public MissionSetting(float altitudePrefered, double area, boolean useSimulator, boolean exitMissionOnRCSignalLostEnabled, @Nullable LatLng homeLocation,boolean autoReturnOnLowBattery, int mLighting){
        ...
        this.area = Utils.clone(area);
        this.lighting = mLighting;
    ...
    public float getSpeed() {
        return Utils.clone(speed);
    }
    public float getMaxSpeed() {
        return Utils.clone(14);
    }
                                ...
                                if (androidPhoneLat == Constants.DOUBLE_NULL || androidPhoneLng == Constants.DOUBLE_NULL){
                                    ...
                                    if (androidPhoneLat.isNaN() || androidPhoneLng.isNaN()){
                                        ...
                                        if (lighting != Constants.DAYSLIGHT_NORMAL && lighting != Constants.DAYSLIGHT_SUNNY){
                                            setErrorMsg ("lighting == 0  No DayLight Selected");
                                            return false;
                                        }else {
                                            if (!includeFlyPoints){
                                                return true;
    ...
    public @Nullable String toStringJSON() throws Exception,Error{
        try {
            if (!isValid(null)){
                ...
                o.put(Constants.JS_KEY_FLY_AREA,getArea());
                o.put(Constants.JS_KEY_DAY_LIGHT,getLighting());
                return o.toString();
            }
        }catch (Exception e){
    ...
    public boolean fillFromStringOk(@NonNull String data){
        if (Utils.isNullOrEmpty(data)){
            ...
            try {
                ...
                setLighting(                        o.getInt(Constants.JS_KEY_DAY_LIGHT));
                setImageCount(                      o.getInt(Constants.JS_KEY_IMAGE_COUNT));
                setArea(                            o.getDouble(Constants.JS_KEY_FLY_AREA));
